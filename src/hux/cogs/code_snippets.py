...
class CodeSnippets(Cog):
    ...
    async def _fetch_pastebin_snippets(
        self, paste_id: str, selections: str
    ) -> list[str]:
        """Fetches snippets from paste.pythondiscord.com."""
        paste_data = await self._fetch_response(
            f"https://paste.pythondiscord.com/api/v1/paste/{paste_id}", "json"
        )
    ...
    def _snippet_to_codeblock(
        ...
        if language is None:
            ...
            trimmed_language = (
                language.replace("-", "").replace("+", "").replace("_", "")
            )
